...
d3.json(earthquakeUrl, function(d) {
	var earthquakeData = d;
	// perform a second GET request to the tectonic plate URL
	d3.json(plateUrl, function(d) {
		var plateData = d;
...
function createFeatures(earthquakeData, plateData) {
	...
	// define a function to draw tectonic plate boundaries
	function onEachLine(feature, layer) {
		L.polyline(feature.geometry.coordinates);
	}
      
    // create GeoJSON layers containing the features array on the earthquakeData & plateData objects
...
function createMap(earthquakes, plates) {
    ...
    var map = L.map("map", {
		center: [37.09, -95.71],
		zoom: 3,
		layers: [outdoors, earthquakes, plates]
    });
